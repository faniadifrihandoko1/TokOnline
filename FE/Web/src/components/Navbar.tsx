...
export default function Navbar() {
    ...
    return (
        <header className="w-full bg-white md:bg-transparent fixed top-0 left-0 right-0 z-50">
            <nav className={`flex justify-between items-center content-center px-5 py-3 text-black lg:py-4 ${isSticky ? "sticky top-0 left-0 border-b bg-white duration-300" : "md:text-white"}`}>
                <div className="flex gap-5 items-center">
                    ...
                    {navLinks.map((data, index) => (
                        <Link key={index} to={data.link} className="text-base font-semibold hidden lg:block   hover:underline hover:underline-offset-8 hover:duration-300 hover:transition-all">
                            {data.name}
                        </Link>
                    ))}
                </div>
                ...
                <div className={clsx("fixed h-full w-screen lg:hidden bg-black/50 backdrop-blur-sm top-0 right-0 -translate-x-full transition-all", isOpen && "translate-x-0")}>
                    <section className="text-black bg-white flex flex-col absolute right-0 top-0 h-screen  py-4 px-5 gap-8 z-50 w-3/4">
                        <div className="flex items-end justify-end">
                            <IoMdClose onClick={() => setIsOpen(false)} className="text-2xl cursor-pointer" />
                        </div>
                        ...
                        <div className="flex flex-col gap-5">
                            {navLinks.map((data, index) => (
                                <Link key={index} to={data.link} className="text-base font-semibold  lg:block  text-gray-800 hover:text-black">
                                    {data.name}
                                </Link>
                            ))}
                        </div>
                ...
                <section className="flex gap-5  items-center">
                    ...
                    <FiMenu onClick={() => setIsOpen(true)} className="text-2xl lg:hidden cursor-pointer" />
                </section>
            </nav>
            {/* <hr /> */}
        </header>
