...
export default function NewProduct() {
    return (
        <section className="px-5 my-10">
            <h1 className="text-3xl font-bold italic text-fontColorBlueSky">New Items</h1>
                    ...
                    {dataPopulerProduct.map((item, index) => (
                        <div className="rounded-lg " key={index}>
                            <div className="p-5 flex flex-col justify-center items-center relative">
                                {/* sale */}
                                <div className={`bg-sale px-2 py-1 text-white absolute top-6 left-0 md:top-14 md:left-5 1 ${item.isReady ? "hidden" : "block"}`}>
